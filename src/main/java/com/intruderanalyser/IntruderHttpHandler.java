...
<head>
    ...
    <style>
        ...
        .table-pane { height: 40vh; border-bottom: 2px solid #dee2e6; }
        .details-pane { height: 40vh; }
        ...
        .histogram-controls { position: absolute; top: 5px; right: 10px; z-index: 10; font-size: 0.9em; background: rgba(255,255,255,0.7); padding: 2px 5px; border-radius: 3px; display: flex; align-items: center; gap: 10px;}
        .histogram-info { font-weight: bold; color: #495057; margin-right: 10px; }
        .run-info { font-size: 0.85em; color: #6c757d; display: flex; gap: 15px; }
        ...
        .remove-tag { margin-left: 5px; font-weight: bold; padding: 0 4px; border-radius: 50%; }
        ...
        th, td { padding: 6px 10px; border: 1px solid #ddd; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 250px; }
    ...
    <script type="text/babel">
        ...
        const AnalysisPane = (props) => (
            <div className="pane-content">
                ...
                <div className="analysis-layout">
                    <div className="status-summary-container"><StatusCodeSummary data={props.allRequests} {...props} /></div>
                    <div className="string-filters-pane"><StringFilters {...props} /></div>
                    <div className="histogram-container"><Histogram data={props.allRequests} {...props} /></div>
                </div>
            </div> 
        );
        const StatusCodeSummary = ({ data, selectedStatusCodes, onStatusSelect }) => {
            ...
            return ( <div className="scrollable-table"><table><thead><tr><th>Status Code</th><th>Count</th></tr></thead><tbody>{sortedStatuses.map(status => ( <tr key={status} className={`clickable ${selectedStatusCodes.includes(Number(status)) ? 'selected' : ''}`} onClick={() => onStatusSelect(Number(status))}><td>{status}</td><td>{statusCounts[status]}</td></tr> ))}</tbody></table></div> );
        };
        const Histogram = ({ data, selectedRanges, onRangeSelect, onSelectAllRanges }) => {
            ...
            const { labels, datasets, allRanges } = useMemo(() => {
                ...
                const maxLength = Math.max(...data.map(r => r.totalResponseLength), 0);
        ...
        const RequestsTable = ({ requests, selectedRequest, onRowSelect, sortConfig, onSort, onTableKeyDown }) => {
            ...
            return ( <div className="scrollable-table" onKeyDown={onTableKeyDown} tabIndex="0"><table><thead><tr>
                <th onClick={() => onSort('id')}>ID <span className="sort-indicator">{getIndicator('id')}</span></th>
                ...
                <th onClick={() => onSort('totalResponseLength')}>Total Length <span className="sort-indicator">{getIndicator('totalResponseLength')}</span></th>
                <th onClick={() => onSort('responseLength')}>CT Length <span className="sort-indicator">{getIndicator('responseLength')}</span></th>
        ...
        function App() {
            ...
            useEffect(() => {
                ...
                ws.onmessage = (event) => {
                    try {
                        ...
                        switch (message.type) {
                            ...
                            case 'result':
                                if (message.data) {
                                    setAllRequests(prev => [...prev, message.data]);
                                }
                                break;
                            case 'runEnd':
                ...
                ws.onerror = (error) => { console.error('WebSocket error:', error); alert('Could not connect to WebSocket server at ws://127.0.0.1:8888'); };
                ws.onclose = () => console.log('WebSocket connection closed.');
                return () => { ws.close(); };
            }, []);
            
            const requestsWithSearchText = useMemo(() => {
                return allRequests.map(req => {
                    const searchText = (decodeBase64Utf8(req.requestBase64) + decodeBase64Utf8(req.responseBase64)).toLowerCase();
                    return { ...req, searchText };
                });
            }, [allRequests]);
            ...
            const handleResetFilters = () => { setSelectedStatusCodes([]); setSelectedRanges([]); setIncludeFilters([]); setExcludeFilters([]); setSelectedRequest(null); };
            ...
            const handleAddFilter = (type, text) => {
                ...
                if (type === 'include') {
                    if (!includeFilters.some(f => f.text.toLowerCase() === text.toLowerCase())) setIncludeFilters(prev => [...prev, newFilter]);
                } else {
                    if (!excludeFilters.some(f => f.text.toLowerCase() === text.toLowerCase())) setExcludeFilters(prev => [...prev, newFilter]);
                }
            };
            const handleToggleFilter = (id) => {
                const toggle = (filters) => filters.map(f => f.id === id ? { ...f, active: !f.active } : f);
                if (includeFilters.some(f => f.id === id)) {
                    setIncludeFilters(toggle);
                } else {
                    setExcludeFilters(toggle);
                }
            };
            const handleRemoveFilter = (id) => {
                if (includeFilters.some(f => f.id === id)) {
                    setIncludeFilters(prev => prev.filter(f => f.id !== id));
                } else {
                    setExcludeFilters(prev => prev.filter(f => f.id !== id));
                }
            };
            ...
            const requestsForTable = useMemo(() => {
                ...
                return requestsWithSearchText.filter(req => {
                    ...
                    if (!statusMatch || !rangeMatch) return false;
            ...
            }, [requestsWithSearchText, selectedStatusCodes, selectedRanges, includeFilters, excludeFilters]);
            ...
            return (
                <div className="app-container">
                    ...
                    <div className="table-pane"><div className="pane-content"><div className="pane-header" style={{flexShrink: 0}}><h3 className="pane-title">Requests ({sortedRequestsForTable.length})</h3></div><RequestsTable requests={sortedRequestsForTable} selectedRequest={selectedRequest} onRowSelect={setSelectedRequest} sortConfig={sortConfig} onSort={handleSort} onTableKeyDown={handleTableKeyDown} /></div></div>
